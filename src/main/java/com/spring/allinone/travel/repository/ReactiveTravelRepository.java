...
public class ReactiveTravelRepository {
	...
	public Mono<List<TravelDto>> getTravels(String userId) {
		return Mono.defer(() -> Mono.just(travelRepository.findByUserId(userId)).subscribeOn(Schedulers.elastic())
	...
	public Mono<TravelDto> addTravel(final TravelDto travel) {
		return Mono.just(travel).map(t -> new Travel(t.getUserId(), t.getSource(), t.getDestination()))
				.publishOn(Schedulers.parallel()).doOnNext(travelRepository::save)
